...
install: all
	for file in $(SCALED); do \
		install -D -m 0644 "$$file" "$(DESTDIR)$(datadir)/backgrounds/pop-hp/$$(basename "$$file")"; \
	done
	install -D -m 0644 "scaled/info.xml" "$(DESTDIR)$(datadir)/gnome-background-properties/pop-hp-wallpapers.xml"

uninstall:
	for file in $(SCALED); do \
		rm -f "$(DESTDIR)$(datadir)/backgrounds/pop-hp/$$(basename "$$file")"; \
	done
	rmdir --ignore-fail-on-non-empty "$(DESTDIR)$(datadir)/backgrounds/pop-hp/"
	rm -f "$(DESTDIR)$(datadir)/gnome-background-properties/pop-hp-wallpapers.xml"
...
scaled/info.xml: $(SCALED)
	...
	for file in $(SCALED); do \
		...
		echo "        <filename>/usr/share/backgrounds/pop-hp/$$(basename "$$file")</filename>" >> "build/info.xml"; \
